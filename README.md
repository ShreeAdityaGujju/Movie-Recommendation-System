...
Movie Recommendation System is a Streamlit application that recommends movies based on the textual content of film metadata. It uses a hybrid approach that combines TF–IDF, Latent Semantic Analysis (SVD), and Sentence‑BERT similarity to generate personalized recommendations. Users can select one or more favourite movies, adjust the weights of each similarity metric, choose the number of recommendations, and view poster images of the suggested films.
